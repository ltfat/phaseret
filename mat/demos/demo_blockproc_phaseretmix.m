...
dlMaxLen = 10*bufLen;
delayline = zeros(dlMaxLen,1);
delay = ceil(fs*1/(1000));
f2ola = [];
...
while flag && p.flag
    ...
    delayline(delay:delay+L-1) = f;
    
    f2 = delayline(1:L);
    
    delayline(1:end-L) = delayline(L+1:end); 
    ...
    if do_bypass || algno==0
        ...
        elseif algno==2
            % Do SPSI
            [cnativeout,tmp] = comp_spsireal(s,a,M,[],startphase);
            startphase = tmp(:,end);
        elseif algno==3
            % Do RTISI-LA
            for ii=1:size(s,2)
