...
PHASERET_API int PHASERET_NAME(rtisila_set_lookahead)(
    PHASERET_NAME(rtisila_state) * p, ltfat_int lookahead)
{
    int status = LTFATERR_SUCCESS;
    CHECKNULL(p);
    CHECK(LTFATERR_BADARG, lookahead >= 0 && lookahead <= p->lookahead,
        "lookahead can only be in range [0-%d] (passed %d).", p->lookahead, lookahead);
...
void PHASERET_NAME(overlaynthframe)(const LTFAT_REAL* frames, ltfat_int gl, ltfat_int N,
    ltfat_int a, ltfat_int n, LTFAT_REAL* frame)
{
    // Copy n-th frame
...
void PHASERET_NAME(rtisilaoverlaynthframe)(PHASERET_NAME(rtisilaupdate_plan) * p,
...
void PHASERET_NAME(rtisilaphaseupdate)(PHASERET_NAME(rtisilaupdate_plan) * p,
...
PHASERET_API int PHASERET_NAME(rtisilaupdate_init)(const LTFAT_REAL* g,
    const LTFAT_REAL* specg1, const LTFAT_REAL* specg2, const LTFAT_REAL* gd,
    ltfat_int gl, ltfat_int a, ltfat_int M, PHASERET_NAME(rtisilaupdate_plan) * *pout)
{
    int status = LTFATERR_SUCCESS;
...
PHASERET_API int PHASERET_NAME(rtisilaupdate_done)(PHASERET_NAME(rtisilaupdate_plan) * *p)
{
    int status = LTFATERR_SUCCESS;
    ...
    if (pp->frame)
        ltfat_free(pp->frame);
    if (pp->fftframe)
        ltfat_free(pp->fftframe);
    if (pp->fwdp)
        LTFAT_NAME(fftreal_done)(&pp->fwdp);
    if (pp->backp)
        LTFAT_NAME(ifftreal_done)(&pp->backp);
    ltfat_free(pp);
    pp = NULL;
error:
    return status;
...
PHASERET_API void PHASERET_NAME(rtisilaupdate_execute)(
...
void PHASERET_NAME(rtisilaupdate)(const LTFAT_REAL* frames, const LTFAT_REAL* g,
    const LTFAT_REAL* specg1, const LTFAT_REAL* specg2, const LTFAT_REAL* gd,
...
void PHASERET_NAME(rtisilaupdatecoef)(const LTFAT_REAL* frames, const LTFAT_REAL* g,
    const LTFAT_REAL* specg1, const LTFAT_REAL* specg2, const LTFAT_REAL* gd,
    ltfat_int gl, ltfat_int a, ltfat_int M, ltfat_int N, const LTFAT_REAL* s,
    ltfat_int lookahead, ltfat_int maxit, LTFAT_REAL* frames2, LTFAT_COMPLEX* c)
{
    PHASERET_NAME(rtisilaupdate_plan)* p = NULL;
    PHASERET_NAME(rtisilaupdate_init)(g, specg1, specg2, gd, gl, a, M, &p);
    PHASERET_NAME(rtisilaupdate_execute)(p, frames, N, s, lookahead, maxit, frames2, c);
    PHASERET_NAME(rtisilaupdate_done)(&p);
}

PHASERET_API int PHASERET_NAME(rtisila_reset)(PHASERET_NAME(rtisila_state) * p)
{
    ltfat_int N, W, gl, M2;
    int status = LTFATERR_SUCCESS;
    CHECKNULL(p);
...
PHASERET_API int PHASERET_NAME(rtisila_init)(const LTFAT_REAL* g, ltfat_int gl,
    ...
    PHASERET_NAME(rtisila_state) * *pout)
{
    ...
    CHECK(LTFATERR_BADARG, lookahead >= 0, "lookahead >=0 failed (passed %d)", lookahead);
    CHECK(LTFATERR_NOTPOSARG, maxit > 0, "maxit must be positive (passed %d)", maxit);
    ...
    CHECKSTATUS(LTFAT_NAME(gabdual_painless)(g, gl, a, M, (LTFAT_REAL*)p->uplan->gd),
        "gabdual failed");
    ...
    PHASERET_NAME(overlaynthframe)(wins, gl, winsNo, a, 0, (LTFAT_REAL*)p->uplan->specg2);

    // Spec1
    memset(wins, 0, gl * sizeof *wins);
    PHASERET_NAME(overlaynthframe)(wins, gl, winsNo, a, 0, (LTFAT_REAL*)p->uplan->specg1);

    ltfat_free(wins);
    wins = NULL;

    CHECKMEM(p->frames = LTFAT_NAME_REAL(calloc)(gl * (lookback + 1 + maxLookahead) * W));
    CHECKMEM(p->s = LTFAT_NAME_REAL(calloc)(M2 * (1 + maxLookahead) * W));

    p->garbageBinSize = 4;
    CHECKMEM(p->garbageBin = (void**)ltfat_malloc(p->garbageBinSize * sizeof(void*)));
...
PHASERET_API int PHASERET_NAME(rtisila_init_win)(LTFAT_FIRWIN win, ltfat_int gl,
    ltfat_int W, ltfat_int a, ltfat_int M, ltfat_int lookahead, ltfat_int maxit,
    PHASERET_NAME(rtisila_state) * *pout)
{
    LTFAT_REAL* g = NULL;
    ...
    initstatus = PHASERET_NAME(rtisila_init)(g, gl, W, a, M, lookahead, maxit, pout);
...
PHASERET_API int PHASERET_NAME(rtisila_done)(PHASERET_NAME(rtisila_state) * *p)
{
    int status = LTFATERR_SUCCESS;
    ...
    CHECKSTATUS(
        PHASERET_NAME(rtisilaupdate_done)(&pp->uplan), "rtisilaupdate done failed");
...
PHASERET_API int PHASERET_NAME(rtisila_execute)(
...
PHASERET_API int PHASERET_NAME(rtisilaoffline)(const LTFAT_REAL s[], const LTFAT_REAL g[],
    ltfat_int L, ltfat_int gl, ltfat_int W, ltfat_int a, ltfat_int M, ltfat_int lookahead,
    ltfat_int maxit, LTFAT_COMPLEX c[])
{
    int status = LTFATERR_SUCCESS;
    PHASERET_NAME(rtisila_state)* p = NULL;
    ltfat_int N = L / a;
