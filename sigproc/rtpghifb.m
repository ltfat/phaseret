function [c,newphase,tgrad,fgrad]=rtpghifb(s,a,fc,tfr,varargin)
...
[tgrad,fgrad] = ...
      ...
      abss,N(1),a,M,tfr,fc,flags.do_real,0);%flags.do_gabor...0
...
fgrad = -pi*fgrad.';
tgrad = a(1)*(tgrad.' + fc') * pi;%scale to samples

for n=1:N
    idx = mod( n-1-1:n-1, N ) + 1;
    nprev = mod( n-2, N ) + 1;
    newphase(:,n) = comp_rtpghifbupdate(logs(:,idx),tgrad(:,idx),fgrad(:,n),newphase(:,nprev),tol,M);
