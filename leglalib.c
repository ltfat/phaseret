        ...
        int flags)
{
    ...
    {
        retval.flags |= MOD_STEPWISE; // Set the default flag if none of the others is set
    }
        ...
        int flags)
{
    ...
    plan->bufr=aligned_alloc(ALIGNBYTES,((M2+kernh-1)*(plan->N+kernw-1))*sizeof*plan->bufr);
    plan->bufi=aligned_alloc(ALIGNBYTES,((M2+kernh-1)*(plan->N+kernw-1))*sizeof*plan->bufi);

    for(int n=0;n<plan->kernelNo;n++)
    {
        plan->kr[n] = aligned_alloc(ALIGNBYTES,plan->plan_col.kernw*plan->plan_col.kernwskip);
        plan->ki[n] = aligned_alloc(ALIGNBYTES,plan->plan_col.kernw*plan->plan_col.kernwskip);
        kernphasefi(kernr,kerni,kernh,kernw,plan->plan_col.kernwskip,n,a,M,plan->kr[n],plan->ki[n]);
    }
        ...
        mwSignedIndex kernwskip, double* kernmodr, double* kernmodi)
{
    ...
    {
        memcpy(kernmodr + ii*kernwskipd,kernr + ii*kernh,kernh2*sizeof*kernmodr);
        memcpy(kernmodi + ii*kernwskipd,kerni + ii*kernh,kernh2*sizeof*kernmodi);
    }
