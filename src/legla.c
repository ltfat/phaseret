...
/*     int N = L / a; */
/*  */
/*     dgtreal_anasyn_init(g, gl, L, W, a, M, c, hint, flags, &p->dgtplan); */
/*     memset(c, 0, M2 * N * W * sizeof * c); */
/*     c[0] = 1.0; */
/*     dgtreal_anasyn_execute_proj(p->dgtplan, c, c); */
/*     complex double* kernsmall = */
...
/*  */
/*     p->updateplan = leglaupdate_init( a, M, N, kernr, kerni, kernh, kernw, */
/*                                      leglaflags); */
/*  */
/* } */
/*  */
...
/*  */
/* int */
/* legla_execute(legla_plan* p, const double s[], const int iter) */
/* { */
...
/*     { */
/*         leglaupdatereal_execute(p->updateplan,  pp->c, pp->c); */
/*  */
/*         if (p->do_fast) */
...
extern void
leglaupdatereal_execute(leglaupdate_plan* plan, const double* s,
                        double* cr, double* ci, double* coutr, double* couti)
{
    int M2 = plan->plan_col.M / 2 + 1;
    int N = plan->N;
    int kernh = plan->plan_col.kernh;
    int kernw2 = plan->plan_col.kernw2;
...
void
kernphasefi(double* kernr, double* kerni, int kernh,
            int kernw,
            int kernwskip,
            int n, int a, int M,
            double* kernmodr, double* kernmodi)
{
    ...
    if (n != 0)
    {
        int kernh2 = kernh / 2 + 1;
        int kernwskipd = kernwskip / (sizeof * kernr);
        /*Modulate */
                            ...
                            double* coutrCol, double* coutiCol)
{
    int m, mfirst, mlast;
    int M2 = plan->M / 2 + 1;
    int kernh = plan->kernh;
    int kernw = plan->kernw;
    int kernw2 = plan->kernw2;
    int kernh2 = plan->kernh2;
    int kernskipd = plan->kernwskip / (sizeof * crColFirst);
    int M2buf = M2 + kernh - 1;
